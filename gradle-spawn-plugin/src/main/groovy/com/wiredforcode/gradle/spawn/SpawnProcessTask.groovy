...
class SpawnProcessTask extends DefaultSpawnTask {
    ...
    private int extractPidFromProcess(Process process) {
        def pidField = null
        if(isWindows()){
            println "it is windows"
            pidField = process.class.getDeclaredField('handle')
        }
        else {
            println "it is not windows"
            pidField = process.class.getDeclaredField('pid')
        }
        if(pidField==null){
            throw new GradleException("Unable to extract Pid from process")
        }
